"use client";
// Builder — Architecture-first blockchain app builder (Client Component)
...
const QUICK_ACTIONS: Record<string, { label: string; prompt: string }[]> = {
  ...
  "pump-token": [
    { label: "Change name",       prompt: "Change the pump token name and symbol" },
    { label: "Lower price",       prompt: "Set initial price to 0.0000001 ETH for max virality" },
    { label: "Max buy cap",       prompt: "Add a max buy limit to prevent whales" },
    { label: "Slippage guard",    prompt: "Add 5% slippage protection on each trade" },
    { label: "Add description",   prompt: "Add a compelling description about community-driven price discovery" },
    { label: "Change emoji",      prompt: "Change the token emoji to 🚀" },
    { label: "Steeper curve",     prompt: "Make the bonding curve steeper so price rises faster" },
    { label: "Dark theme",        prompt: "Switch to a dark neon theme" },
    { label: "Viral tagline",     prompt: "Write a viral one-liner tagline for the token" },
  ],
  "nft-mint": [
    { label: "Free mint",         prompt: "Make this a free mint — set price to 0 ETH" },
    { label: "Change price",      prompt: "Change the mint price" },
    { label: "Boost supply",      prompt: "Set max supply to 10,000 NFTs" },
    { label: "Batch mint",        prompt: "Allow minting up to 20 at once" },
    { label: "Pause minting",     prompt: "Add ability to pause and resume minting" },
    { label: "Reveal mechanic",   prompt: "Add a delayed reveal mechanic for mystery boxes" },
    { label: "Whitelist phase",   prompt: "Add a whitelist-only phase before public mint" },
    { label: "Refund policy",     prompt: "Add a refund mechanism if mint fails" },
    { label: "Airdrop owners",    prompt: "Add an owner airdrop function for giveaways" },
  ],
  "token-gated": [
    { label: "Change title",      prompt: "Change the page title to Alpha Holders Lounge" },
    { label: "Higher min bal",    prompt: "Set minimum token balance to 100" },
    { label: "NFT gate",          prompt: "Gate access using NFT ownership instead of ERC-20 balance" },
    { label: "Multi-token",       prompt: "Allow access with any of 3 different tokens" },
    { label: "Time-limited",      prompt: "Add time-limited access that expires after 30 days" },
    { label: "Change description",prompt: "Write a compelling teaser about the exclusive content" },
    { label: "Gold theme",        prompt: "Change color scheme to gold for premium feel" },
    { label: "Access log",        prompt: "Log access attempts on-chain for analytics" },
    { label: "Revoke access",     prompt: "Add ability to revoke specific wallet access" },
  ],
  "click-to-earn": [
    { label: "Higher reward",     prompt: "Increase reward per click to 0.001 ETH" },
    { label: "Longer cooldown",   prompt: "Set cooldown to 60 seconds between clicks" },
    { label: "Add leaderboard",   prompt: "Add a top-10 daily leaderboard stored on-chain" },
    { label: "Referral system",   prompt: "Add a referral system where referrers earn 10% bonus" },
    { label: "Combo multiplier",  prompt: "Add a combo multiplier — rapid taps earn 2x" },
    { label: "Season rewards",    prompt: "Add weekly seasons with bonus prize pool" },
    { label: "Change game name",  prompt: "Rename the game to CryptoClicker" },
    { label: "Add achievements",  prompt: "Add milestone achievements — 100, 1000, 10000 clicks" },
    { label: "Anti-bot check",    prompt: "Add proof-of-humanity check to prevent bot clicking" },
  ],
};

// ── Main Page ─────────────────────────────────────────────────────────────────

export default function BuilderClient({ params, initialProject }: { params: { slug: string }; initialProject: Project }) {
  ...
  const NETWORKS = [
    // Mainnets — real money
    { id: 1,        hex: "0x1",       name: "Ethereum",       nativeCurrency: "ETH",   explorer: "https://etherscan.io",           rpc: "https://eth.llamarpc.com",             type: "mainnet" as const },
    { id: 8453,     hex: "0x2105",    name: "Base",           nativeCurrency: "ETH",   explorer: "https://basescan.org",            rpc: "https://mainnet.base.org",             type: "mainnet" as const },
    { id: 137,      hex: "0x89",      name: "Polygon",        nativeCurrency: "MATIC", explorer: "https://polygonscan.com",         rpc: "https://polygon-rpc.com",              type: "mainnet" as const },
    { id: 42161,    hex: "0xa4b1",    name: "Arbitrum One",   nativeCurrency: "ETH",   explorer: "https://arbiscan.io",             rpc: "https://arb1.arbitrum.io/rpc",         type: "mainnet" as const },
    { id: 10,       hex: "0xa",       name: "Optimism",       nativeCurrency: "ETH",   explorer: "https://optimistic.etherscan.io", rpc: "https://mainnet.optimism.io",          type: "mainnet" as const },
    { id: 56,       hex: "0x38",      name: "BNB Smart Chain",nativeCurrency: "BNB",   explorer: "https://bscscan.com",             rpc: "https://bsc-dataseed.binance.org",     type: "mainnet" as const },
    // Testnets — free test ETH, no real value
    { id: 11155111, hex: "0xaa36a7",  name: "Sepolia",        nativeCurrency: "ETH",   explorer: "https://sepolia.etherscan.io",    rpc: "https://rpc.sepolia.org",              type: "testnet" as const },
    { id: 84532,    hex: "0x14a34",   name: "Base Sepolia",   nativeCurrency: "ETH",   explorer: "https://sepolia.basescan.org",    rpc: "https://sepolia.base.org",             type: "testnet" as const },
    { id: 80001,    hex: "0x13881",   name: "Polygon Mumbai", nativeCurrency: "MATIC", explorer: "https://mumbai.polygonscan.com",  rpc: "https://rpc-mumbai.maticvigil.com",    type: "testnet" as const },
  ] as const;
  type NetworkEntry = (typeof NETWORKS)[number];

  const [selectedNetworkId, setSelectedNetworkId] = useState<number>(11155111); // default Sepolia
  const selectedNetwork: NetworkEntry = NETWORKS.find((n) => n.id === selectedNetworkId) ?? NETWORKS[6];
  const onCorrectChain = ethChainId === selectedNetworkId;
  ...
  const [config, setConfig]         = useState<Record<string, string>>(initialProject.paramValues ?? {});
  const [templateId, setTemplateId] = useState<TemplateId>((initialProject.paramValues?._templateKey ?? "erc20-token") as TemplateId);
  ...
  // Project is loaded server-side and passed as initialProject prop — no fetch needed
            ...
            <div className="flex-1 overflow-auto bg-white">
              <div className="max-w-xl mx-auto px-6 py-8">
                ...
                <div className="bg-white border border-gray-200 rounded-2xl p-5 mb-4 space-y-3">
                  ...
                  {deployTxHash && (
                    <div>
                      ...
                      <div className="flex items-center gap-2 bg-gray-50 rounded-xl px-3 py-2">
                        ...
                        <a href={`{selectedNetwork.explorer}/tx/${deployTxHash}`} target="_blank" rel="noopener noreferrer" title="View on Etherscan">
                  ...
                  {deployedAddress && (
                    <a href={`{selectedNetwork.explorer}/address/${deployedAddress}`} target="_blank" rel="noopener noreferrer"
                      className="inline-flex items-center gap-1 text-xs text-indigo-600 hover:text-indigo-800 font-medium">
                      View on ${selectedNetwork.name} Explorer <ExternalLink className="w-3 h-3" />
                    </a>
                  )}
                </div>
              ...
              <div className={`max-w-md space-y-4 ${isDeployed ? "pointer-events-none select-none opacity-75" : ""}`}>
                ...
                <div className="flex items-center">
                  {[
                    ...
                    { label: `Switch to ${selectedNetwork.name}`, done: isConnected && onCorrectChain },
                ...
                {isConnected && (
                  <div className="border border-gray-200 bg-white rounded-2xl overflow-hidden shadow-sm">
                    <div className="px-4 py-3 border-b border-gray-100">
                      <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Step 2 · Choose Network</p>
                      <p className="text-xs text-gray-400 mt-0.5">Pick where you want to deploy your contract</p>
                    </div>
                    ...
                    <div className="px-4 pt-3 pb-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-[10px] font-bold text-emerald-700 uppercase tracking-wider">Mainnet — Real tokens, real value</p>
                      </div>
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5 mb-3">
                        {NETWORKS.filter((n) => n.type === "mainnet").map((n) => (
                          <button
                            key={n.id}
                            onClick={() => setSelectedNetworkId(n.id)}
                            className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-left transition-all text-xs font-medium
                              ${selectedNetworkId === n.id
                                ? "border-emerald-400 bg-emerald-50 text-emerald-700"
                                : "border-gray-200 bg-gray-50 text-gray-600 hover:border-gray-300 hover:bg-white"}`}
                          >
                            <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${selectedNetworkId === n.id ? "bg-emerald-500" : "bg-gray-300"}`} />
                            <span className="truncate">{n.name}</span>
                            <span className="text-gray-400 ml-auto shrink-0">{n.nativeCurrency}</span>
                          </button>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="px-4 pb-3">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-[10px] font-bold text-amber-600 uppercase tracking-wider">Testnet — Free test tokens, not real</p>
                      </div>
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                        {NETWORKS.filter((n) => n.type === "testnet").map((n) => (
                          <button
                            key={n.id}
                            onClick={() => setSelectedNetworkId(n.id)}
                            className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-left transition-all text-xs font-medium
                              ${selectedNetworkId === n.id
                                ...
                                : "border-gray-200 bg-gray-50 text-gray-600 hover:border-gray-300 hover:bg-white"}`}
                          >
                            <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${selectedNetworkId === n.id ? "bg-amber-500" : "bg-gray-300"}`} />
                            <span className="truncate">{n.name}</span>
                            <span className="text-gray-400 ml-auto shrink-0 text-[9px]">TEST</span>
                          </button>
                        ))}
                      </div>
                    </div>
                      ...
                      <div className="px-4 pb-3 pt-1 border-t border-gray-100">
                        <div className="flex items-center justify-between">
                          <p className="text-xs text-gray-400">
                            Current chain: {ethChainId ?? "unknown"}
                          </p>
                          <button
                            onClick={async () => {
                              setDeployError(null);
                              try {
                                await window.ethereum.request({
                                  ...
                                  params: [{ chainId: selectedNetwork.hex }],
                                });
                              } catch (err: unknown) {
                                const msg = err instanceof Error ? err.message : String(err);
                                if (msg.includes("4902") || msg.includes("Unrecognized chain")) {
                                  try {
                                    await window.ethereum.request({
                                      ...
                                      params: [{
                                        chainId:            selectedNetwork.hex,
                                        chainName:          selectedNetwork.name,
                                        nativeCurrency:     { name: selectedNetwork.nativeCurrency, symbol: selectedNetwork.nativeCurrency, decimals: 18 },
                                        rpcUrls:            [selectedNetwork.rpc],
                                        blockExplorerUrls:  [selectedNetwork.explorer],
                                      }],
                                    });
                                  } catch (addErr: unknown) {
                                    setDeployError(addErr instanceof Error ? addErr.message : "Failed to add chain");
                                  }
                                } else {
                                  setDeployError(msg);
                                }
                    ...
                    {onCorrectChain && (
                      <div className="px-4 pb-3 pt-1 border-t border-gray-100 flex items-center gap-2">
                        ...
                        <p className="text-xs text-emerald-600 font-medium">
                          Connected to {selectedNetwork.name} · Ready to deploy
                        </p>
                        {selectedNetwork.type === "testnet" && (
                          <span className="ml-auto text-[10px] bg-amber-100 text-amber-700 border border-amber-200 rounded-full px-2 py-0.5">
                            Testnet
                          </span>
                        )}
                      </div>
                ...
                {compiled && compileResult && isConnected && onCorrectChain && (
                  <div className="space-y-3">
                    ...
                    {deployPhase === "confirming" && deployTxHash && (
                      <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <a href={`{selectedNetwork.explorer}/tx/${deployTxHash}`} target="_blank" rel="noopener noreferrer"
                      ...
                      {deployTxHash && (
                        <div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <a href={`{selectedNetwork.explorer}/tx/${deployTxHash}`} target="_blank" rel="noopener noreferrer" className="flex-shrink-0">
